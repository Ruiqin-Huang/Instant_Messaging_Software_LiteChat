...
bool TcpClientSocket::connectToServer()
{
    abort();
    connectToHost(QHostAddress(this->_serverIP),this->_serverTcpPort);
    bool connection_stat = waitForConnected();
    qDebug() << connection_stat;
...
//        qDebug()<<"[TcpClientSocket::receiveMessage] type ="+QString::number(tmp.getType()==MsgType::GROUP_HISTORY_MSG);
